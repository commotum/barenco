...
| Stage 6 warning-as-error root/audit | `lake env lean -DwarningAsError=true Barenco.lean`; `lake env lean -DwarningAsError=true Barenco/AxiomAudit.lean` | both successful; the audit printed 80 declarations, including all 16 new Stage 6 checks, and every result is exactly `propext`, `Classical.choice`, and `Quot.sound`, 2026-07-09 |
| Stage 6 focused/diagnostic/root/audit build | `lake build Barenco.ThreeQubit.Lemma61 Barenco.ThreeQubit.Expansion Barenco.ThreeQubit.RelativePhase Barenco.ThreeQubitExamples Barenco.AxiomAudit Barenco` | successful, 2,947 jobs, 2026-07-09 |
| Stage 6 full builds | two consecutive `lake build` runs after the public-root integration | both successful, 2,945 jobs each, 2026-07-09 |
