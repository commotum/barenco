...
## Lemma 7.7 Lower-Bound Conventions

The lower bound does not quantify over the generic `Circuit` type, whose
primitive vocabulary deliberately includes macros and unclassified certified
matrices. `LowerBounds.BasicPrimitive n` is instead proof-carrying syntax with
exactly two constructors: an arbitrary certified one-qubit gate on one wire, and
a CNOT carrying a proof that its ordered control and target are distinct.
`BasicCircuit n` is a chronological list of these primitives. Erasure uses only
the trusted `Primitive.oneQubit` and `Primitive.cnot` smart constructors, and
`BasicCircuit.eval_erase` proves exact agreement with the generic evaluator.
Separate syntax recursors compute one-qubit, CNOT, and total occurrence counts;
their erasure bridges establish the exact `CostModel.oneQubitCNOT` value.

`BasicCircuit.interactionGraph` is the undirected simple graph generated by CNOT
endpoint pairs. Chronology and repeated CNOTs are retained in `cnotEdges`, then
forgotten when its edge list is converted to a simple graph. Consequently the
number of distinct graph edges is *at most* the CNOT occurrence count. It is never
identified with that count. A connected interaction graph on `Fin n` therefore
forces `n-1 <= cnotCount`; one-qubit gates do not contribute graph edges even
though arbitrarily many remain available.

For any `cut : Finset (Fin n)`, including a noncontiguous set of wires,
`wireSplit cut` sends a computational-basis assignment to the pair consisting of
its restriction to `cut` and its restriction to the complement. The selected
side is always the first coordinate. `partitionReindex cut` applies this same
equivalence to both matrix axes, and `TensorFactorsAcross cut G` means that the
reindexed raw matrix is exactly `left ⊗ₖ right` in selected-first order. The
factors are not required to be certified unitaries; making factorability weaker
makes the nonfactorization result stronger. Same-side one-qubit gates and CNOTs
are proved tensor-local, and `Matrix.mul_kronecker_mul` preserves factorization in
the evaluator's chronological multiplication order.

When the interaction graph is disconnected, the lower-bound proof chooses the
connected component containing the target. This orientation guarantees that the
target lies on the selected side and some other wire lies outside it, so no
separate definitional tensor-swap convention is hidden. Every primitive remains
on one side, and `eval_tensorFactorsAcross_targetComponent` proves exact
factorization of the full-register evaluator—not merely independence of its
classical basis action.

`IsScalarQubitMatrix U` means `U = z • I` as a raw two-by-two matrix. For a
certified `QubitUnitary`, this is equivalent to `U = phaseShiftUnitary delta` for
some real `delta`. Explicit active/inactive basis assignments prove that if a
listed positive control is separated from the target and the controlled matrix
factors across the cut, then the target matrix is scalar. Thus the headline
`fullyControlled_cnotCount_lowerBound` assumes exact evaluator equality to a
fully positive-controlled *nonscalar* target and concludes at least `n-1` CNOT
occurrences. `fullyControlled_gateCount_lowerBound` is the paper's weaker total
basic-operation consequence, and
`fullyControlled_oneQubitCNOTCost_lowerBound` is its accepted named-cost form.

These are same-register, no-ancilla lower bounds. They do not cover clean or dirty
auxiliary wires, approximation, measurements, global- or basis-phase-relaxed
targets, Toffoli or arbitrary-two-qubit primitives, or a circuit with an
unclassified semantic node. Width one gives the trivial bound zero. Width zero
has no `target : Fin 0`, so it cannot instantiate the theorem. Scalar targets are
intentionally outside the obstruction: controlling a scalar phase creates a
control-dependent diagonal phase but no target dependence. Finally, the result
does not turn the exact Section 5 family classifications, Section 5/6 constructed
